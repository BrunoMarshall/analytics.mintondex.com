...
import { Pair, Token, Swap, Mint as MintEntity, Burn as BurnEntity, PairDayData, MintondexDayData } from "../generated/schema";
...
function updatePairDayData(event: SyncEvent, pair: Pair): void {
  ...
  if (d === null) {
    d = new PairDayData(id);
    d.date = dayIndex * 86400;
    d.pair = pair.id;
    d.volumeToken0 = ZERO_BD;
    d.volumeToken1 = ZERO_BD;
    d.volumeUSD = ZERO_BD;
    d.txCount = BigInt.fromI32(0);
  }
  d.reserve0 = pair.reserve0;
  d.reserve1 = pair.reserve1;
  d.save();
}
...
export function handleBurn(event: BurnEvent): void {
  ...
  burn.sender = event.params.to;
  burn.amount0 = amt0;
  burn.amount1 = amt1;
  burn.amountUSD = amtUSD;
...
export function handleSwap(event: SwapEvent): void {
  ...
  let vol0 = amt0In.plus(amt0Out);
  let vol1 = amt1In.plus(amt1Out);
  ...
  pair.txCount = pair.txCount.plus(BigInt.fromI32(1));
  t0.tradeVolume = t0.tradeVolume.plus(vol0);
  t0.txCount = t0.txCount.plus(BigInt.fromI32(1));
  t1.tradeVolume = t1.tradeVolume.plus(vol1);
  t1.txCount = t1.txCount.plus(BigInt.fromI32(1));
  t0.save();
  t1.save();
  pair.save();
  ...
  swap.sender = event.params.to;
  swap.to = event.params.sender;
  swap.amount0In = amt0In;
  swap.amount1In = amt1In;
  swap.amount0Out = amt0Out;
  swap.amount1Out = amt1Out;
