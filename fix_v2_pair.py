import io
content = """import { Swap as SwapEvent, Mint as MintEvent, Burn as BurnEvent, Sync as SyncEvent } from "../generated/templates/Pair/Pair";
...
function updateProtocolDayData(event: SyncEvent, volumeUSD: BigDecimal): void {
  let dayIndex = event.block.timestamp.toI32() / 86400;
...
export function handleSync(event: SyncEvent): void {
  ...
  pair.reserve0 = convertToDecimal(event.params.reserve0.toBigInt(), t0.decimals);
  pair.reserve1 = convertToDecimal(event.params.reserve1.toBigInt(), t1.decimals);
...
export function handleMint(event: MintEvent): void {
  ...
  updateProtocolDayData(event as any, ZERO_BD);
}
...
export function handleSwap(event: SwapEvent): void {
  ...
  updateProtocolDayData(event as any, volumeUSD);
}
"""
